...
const ClientsCloud = () => {
  ...
  return (
    <section id="clients" className="py-24 md:py-32 bg-[#020209] relative overflow-hidden">
      {/* Background Glows */}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-7 gap-2 md:gap-4">
          {logos.map((src, i) => (
            <motion.div
              ...
              className="group aspect-square bg-white/[0.03] backdrop-blur-md rounded-xl md:rounded-2xl p-0 flex items-center justify-center border border-white/5 hover:border-primary/40 hover:shadow-[0_20px_50_rgba(99,102,241,0.15)] transition-all duration-500 cursor-pointer overflow-hidden"
            >
              <img 
                src={src} 
                alt={`Client ${i + 1}`} 
                ...
                onError={(e) => {
                  ...
                  e.target.style.opacity = '0.2'; // Keep it but faded if failed
                }}
              />
            </motion.div>
          ))}
